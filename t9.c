...
int main()
{
    ...
    int ary_vi;
    int ary_pos;
    int set_i0, set_i1, set_const, set_pos;
    ...
    while (fgets(line, LINESZ, stdin) != NULL) 
    {
        ...
        //ACESSO AO ARRAY
        //get
        r = sscanf(line, "get %ca%d index ci%d to %ci%d", &ary_ad, &ary_ai, &ary_const, &ary_ad2, &ary_vi);
        if(r == 4){
            if(ary_ad != 'p'){
                for (i = 0; i < i_pilha; i++)
                {
            ...
            if(ary_ad2 != 'p'){
                ...
                if(ary_ai == 1){
                    printf("    movl %%r11d, %%rdi\n");
                    fprintf(arq, "    movl %%r11d, %%rdi\n");
                }

                if(ary_ai == 2){
                    printf("    movl %%r11d, %%rsi\n");
                    fprintf(arq, "    movl %%r11d, %%rsi\n");
                }

                if(ary_ai == 3){
                    printf("    movl %%r11d, %%rdx\n");
                    fprintf(arq, "    movl %%r11d, %%rdx\n");
                }
            }
